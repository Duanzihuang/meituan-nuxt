<template>
  <div class="m-menu">
    <dl class="nav" @mouseleave="mouseleave">
      <dt>全部分类</dt>
      <dd v-for="item in $store.getters['home/getMenus']" :key="item.type" @mouseenter="mouseenter">
        <i :class="item.type"/>{{item.name}}<span class="arrow"></span>
      </dd>
    </dl>
    <div v-if="kind" class="detail" @mouseenter="enter" @mouseleave="leave">
      <div v-for="item in currentDetail.child" :key="item.title">
        <h4>{{item.title}}</h4>
...
export default {
  ...
  computed:{
    currentDetail(){
      ...
      return this.$store.getters['home/getMenus'].filter(item => item.type === this.kind)[0] || {}
    }
  },
  methods:{
    mouseleave(){
